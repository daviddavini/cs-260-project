...
class AGNNConv(nn.Module):
    def __init__(
        ...
        if isinstance(in_feats, tuple):
            ...
            self.fc = nn.Linear(self._in_src_feats, out_feats * num_heads, bias=False)
        self.attn = nn.Parameter(torch.FloatTensor(size=(1, num_heads, 1)))
        #self.attn_l = nn.Parameter(torch.FloatTensor(size=(1, num_heads, out_feats)))
        #self.attn_r = nn.Parameter(torch.FloatTensor(size=(1, num_heads, out_feats)))
        self.feat_drop = nn.Dropout(feat_drop)
        self.attn_drop = nn.Dropout(attn_drop)
    ...
    def forward(self, graph, feat):
        with graph.local_scope():
            ...
            # Get beta to be the right size for applying to the features. We should have one beta for each head.
            beta = self.attn * torch.ones((1,self._num_heads, self._out_feats))

            # beta * h_i / ||h_i||_2 where h_i \in feat_src is a source feature
            el = beta * (feat_src / torch.norm(feat_src, dim=2, keepdim=True))

            #h_j / ||h_j||_2 where h_j \in feat_dst is a neighbor destination feature
            er = feat_dst / torch.norm(feat_dst, dim=2, keepdim=True)

            graph.srcdata.update({"ft": feat_src, "el": el})
            graph.dstdata.update({"er": er})
            # compute edge attention, el and er are a_l Wh_i and a_r Wh_j respectively.
            #Get beta * cos(h_1, h_j) using the dot product
            graph.apply_edges(fn.u_dot_v("el", "er", "e"))
            e = graph.edata.pop("e")

            # No LeakyReLU applied in AGNN paper
            #e = self.leaky_relu(graph.edata.pop("e"))
            # compute softmax
            graph.edata["a"] = self.attn_drop(edge_softmax(graph, e))
            # message passing
...
class AttenNet(nn.Module):
    def __init__(
        self, attention_mechanism, in_feats, n_classes, n_hidden, n_layers, n_heads, activation, dropout=0.0, attn_drop=0.0, norm="none"
    ):
        super().__init__()
        ...
        for i in range(n_layers):
            ...
            if attention_mechanism == 'AGNN':
                self.convs.append(AGNNConv(in_hidden, out_hidden, num_heads=n_heads, attn_drop=attn_drop, norm=norm))
            elif attention_mechanism == 'GeniePath':
...
## Attempt to specify AGNN exactly as described in the paper, but it's not training
...
#         self.linear_start = nn.Linear(in_feats, n_hidden, bias=True)
#         self.linear_end = nn.Linear(n_heads*n_hidden, n_classes, bias=True)

#         for i in range(n_layers):
#             in_hidden = n_heads * n_hidden if i > 0 else n_hidden
#             #in_hidden = n_heads * n_hidden if i > 0 else n_in_feats
#             out_hidden = n_hidden #if i < n_layers - 1 else n_classes
#             # in_channels = n_heads if i > 0 else 1
#             out_channels = n_heads
